...
const GenerateVirtualDressUpInputSchema = z.object({
  ...
  garmentPhotoDataUri: z
    .string()
    .describe(
      "A photo of the garment, as a data URI."
    ),
...
export async function generateVirtualDressUp(input: GenerateVirtualDressUpInput): Promise<GenerateVirtualDressUpOutput> {
  ...
  try {
    ...
    const finalPrompt = `Subistitua a peça de roupa que a modelo estiver usando pela roupa que esta na 2 imagem enviada para vc fazer a subistituição. Descrição da peça: ${garmentDescription}. Guia Positivo: ${positivePrompt}. Guia Negativo (EVITE a todo custo): ${negativePrompt}. ${customStylePrompt ? `Estilo Personalizado: ${customStylePrompt}` : ''}`;

    const result = await openai.images.edit({
      model: "dall-e-2", // Using DALL-E 2 as it's typically used for edit, but can be changed.
      image: new File([modelBuffer.buffer], "model.png", { type: modelBuffer.mimeType }),
      mask: new File([garmentBuffer.buffer], "garment.png", { type: garmentBuffer.mimeType }), // Sending garment as mask might be one approach.
      prompt: finalPrompt,
      size: "1024x1024",
      n: 1,
